...
### native
1. 编译原生可执行文件（需要安装 GraalVM）
```shell             
  mvn clean package -Pnative        
```                                                                                         
2. 编译完成后，会在各模块的 target/ 目录下生成原生可执行文件，直接运行即可（不需要 java -jar）。                                                                                             
  ...
  ```                                                                                                                     
  具体的二进制文件名取决于 pom.xml 中 native-image 的配置。你可以先执行 mvn clean package -Pnative，然后查看target目录下生成的可执行文件。      
